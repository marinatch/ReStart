...
//Since the movement event has more than 16 parameters, a coustomised MovementDelegate should be created
//A Delegate is a reference type, that would hold references to methods. And the methods will have the same signature that the deligate has defined
...
public static class EventHandler
{
    ...
    //Destroy Goal
    public static event Action<Crop> DestroyCropPrefabsEvent;

    public static void CallDestroyCropPrefabsEvent(Crop crop)
    {
        if (DestroyCropPrefabsEvent != null)
        {
            DestroyCropPrefabsEvent(crop);
        }
    }

    // Inventory Updated Event
    public static event Action<InventoryLocation, List<InventoryItem>> InventoryUpdatedEvent;
    //the method that publeshes an event
   ...
   bool idleUp, bool idleDown, bool idleLeft, bool idleRight)
    {
        //Check if there are subscribers for the movment event
