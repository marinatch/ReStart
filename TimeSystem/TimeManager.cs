...
public class TimeManager : SingletonMonobehaviour<TimeManager>, ISaveable
{
    ...
    private void GameTick()
    {
        ...
        if (gameTick >= Settings.secondsPerGameSecond) //could Wile loop be better?
        {
            //clear the gameTick
        ...
        if (gameSecond > 59)
        {
            ...
            if (gameMinute > 59)
            {
                ...
                if (gameHour > 23)
                {
                    ...
                    {
                        gameDay = 1;
                        //1.gameSeason is an enum, so here it's casted into an intreger and is set to the local vareable gs
                        int gs = (int)gameSeason;
                        gs++;
                        //2. the value of gs is casted into an enum of type gameSeason
